...
class BMotorClient:
    ...
    def set_operating_mode(self, motor_ids: Sequence[int], mode: int):
        ...
        B型舵机无此概念，仅打印提示。
        """
        logging.info(f"[BMotorClient] SCServo does not support operating modes. Ignored mode: {mode}")
    ...
    def write_desired_pos(self, motor_ids: Sequence[int], positions: Union[float, Sequence[float]]):
        ...
        position_dict = dict(zip(motor_ids, positions))
        self.write_positions_with_speed_acc_torque(position_dict)
