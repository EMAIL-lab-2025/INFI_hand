...
class OrcaHand:
    ...
    def set_max_current(self, current):
        self.motor.set_max_current(current)
    ...
    def get_motor_temp(self):
        return self.motor.read_temperatures()
            ...
            for sid in motor_ids:
                ...
                while not reached:
                    ...
                    if len(buffer) > self.calib_window:
                        buffer.pop(0)
                    if np.mean(buffer) > self.calib_current:
                        print(f"[Calib] Detected contact. Avg current = {np.mean(buffer):.2f}")
                        reached = True

                zero_positions[sid] = self.motor.read_positions()[sid]
